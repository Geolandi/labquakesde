...
#diff_eq_type = 'param7' # SDE chaotic
...
if Anoise == 0:
	file_name = 'Fig_les_D_TvsT_ToverT_noisefree.png'
else:
	file_name = 'Fig_les_D_TvsT_ToverT_noiseobs.png'
...
#case_studies = ['synthetic/'+diff_eq_type+'/b698']
#%% SET FLAGS
flag_save_fig     = False
flag_load_data    = True
...
if flag_load_results==True:
	...
	for cc,case_study in enumerate(case_studies):
		...
		if flag_load_EVT==True:
			data = \
				load_pickle(\
			    dirs['pickles']+'/EVT_ShearStress_det_'+\
		...
		if flag_load_LEs==True:
			mhat_tmp, tau_delay_tmp, LEs_tmp, LEs_mean_tmp, LEs_std_tmp, \
			    ...
			    dirs['pickles']+'/LEs_ShearStress_det_'+\
					 str('%.2e' %(Anoise))+'.pickle', \
...
plt.grid()
#ax3.plot(np.nan, np.nan, label=r'$<\sigma_{n}>$',c='w')
#ax3.plot(np.nan, np.nan, label=r'[MPa]',c='w')
ax3.plot(np.nan, np.nan, label=r'$\sigma_{n0}$ [MPa]',c='w')
for cc,case_study in enumerate(case_studies):
	ax3.plot(Tpre[case_study], Tnext[case_study], format_color[cc], \
		  label=str('%.3f' %(sigman0_nominal[cc])), \
			  fillstyle='none')
...
if diff_eq_type == 'param4':
	...
	yc = xc
	delta = 45.0  # degrees
	ell = Ellipse((xc, yc), 3.3, 4.0, delta, ls='--', lw=lw, fc='None', ec='orange')
	...
	yc = xc
	delta = 45.0  # degrees
	ell = Ellipse((xc, yc), 4.2, 5.5, delta, ls='--', lw=lw, fc='None', ec='orange')
	ell.set_clip_box(ax3.bbox)
	ax3.add_artist(ell)
	x0 = 8.6
	y0 = x0
	...
	yc = xc
	delta = 45.0  # degrees
	ell = Ellipse((xc, yc), 4.2, 5.2, delta, ls='--', lw=lw, fc='None', ec='orange')
	ell.set_clip_box(ax3.bbox)
	ax3.add_artist(ell)
	x0 = 8.3
	y0 = x0
...
if diff_eq_type == 'param4':
	...
	xc = 20      #x-position of the center
	yc = 1.0     #y-position of the center
	aup1 = 1     #radius on the x-axis
	aup2 = 4     #radius on the x-axis
	bup = 0.52    #radius on the y-axis
	adown1 = 4     #radius on the x-axis
	adown2 = 1     #radius on the x-axis
	bdown = 0.34    #radius on the y-axis
	t = np.linspace(0, np.pi/2, 50)
	ax4.plot(xc + aup1*np.cos(t), yc + bup*np.sin(t), '--', c='orange', lw=lw)
	t = np.linspace(np.pi/2, np.pi, 50)
	ax4.plot(xc + aup2*np.cos(t), yc + bup*np.sin(t), '--', c='orange', lw=lw)
	t = np.linspace(np.pi, (3/2)*np.pi, 50)
	ax4.plot(xc + adown1*np.cos(t), yc + bdown*np.sin(t), '--', c='orange', lw=lw)
	t = np.linspace((3/2)*np.pi, 2*np.pi, 50)
	ax4.plot(xc + adown2*np.cos(t), yc + bdown*np.sin(t), '--', c='orange', lw=lw)
	x0 = 21
	x1 = 25
	t = np.arange(x0,x1,0.1)
	ax4.plot(t, np.ones(t.shape[0]), '--', c='orange', lw=lw)
	...
	t = np.arange(x0,x1,0.1)
	ax4.plot(t, np.ones(t.shape[0]), '--', c='orange', lw=lw)
	xc = 18.5      #x-position of the center
	yc = 1.0     #y-position of the center
	aup1 = 3.5     #radius on the x-axis
	aup2 = 2.5     #radius on the x-axis
	bup = 0.8    #radius on the y-axis
	adown1 = 2.5     #radius on the x-axis
	adown2 = 3.5     #radius on the x-axis
	bdown = 0.5    #radius on the y-axis
	t = np.linspace(0, np.pi/2, 50)
	ax4.plot(xc + aup1*np.cos(t), yc + bup*np.sin(t), '--', c='orange', lw=lw)
	t = np.linspace(np.pi/2, np.pi, 50)
	ax4.plot(xc + aup2*np.cos(t), yc + bup*np.sin(t), '--', c='orange', lw=lw)
...
cbar_ax = fig.add_axes([0.68, 0.475, 0.1, 0.012])
...
ax1.text(x1lim[0]+x_letterb*(x1lim[1]-x1lim[0]), \
		 y1lim[0]+y_letterb*(y1lim[1]-y1lim[0]),'(b)', fontsize=fontsize)
x2lim = ax2.get_xlim()
y2lim = ax2.get_ylim()
ax2.text(x2lim[0]+x_letterc*(x2lim[1]-x2lim[0]), \
		 y2lim[0]+y_letterc*(y2lim[1]-y2lim[0]),'(c)', fontsize=fontsize)
x3lim = ax3.get_xlim()
y3lim = ax3.get_ylim()
